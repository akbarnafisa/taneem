<template>
  <div>
    ...
    <div v-for="(item) in data" :key="item._id" class="item__wrapper">
      <div class="item">
        <div class="item__left">
          <nuxt-link :to="`/produk/taneem-${item.name}`" class="item__image_wrapper">
            <img :src="item.image[0]" class="item__image">
          </nuxt-link>
          <div class="item__desc w-100">
            <div class="item__title flex-wrap">
              Tane'em - {{item.name}}
              <div class="item__price-wrapper flex-wrap">
                <div class="product__price">Rp. {{(item.price).toLocaleString('id') }}</div>
                <div
                  v-if="item.fromDiscount > 0"
                  class="product__discount"
                >Rp. {{(item.fromDiscount).toLocaleString('id') }}</div>
              </div>
            </div>
            <div class="item__color">Warna: {{item.color}}</div>
            <div class="item__size">Ukuruan: {{item.size}}</div>
            <product-quantity v-if="isCartPage" class="mt-2 mb-3" type="cart" :stock="item.stock"/>
            <div class="item__quantity">Banyaknya: {{item.quantity}}</div>
          </div>
        </div>
        <!-- <div class="item__right"></div> -->
      </div>
      <div v-if="isCartPage" class="item__delete">Hapus</div>
    </div>
  </div>
</template>
